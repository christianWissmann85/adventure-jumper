...
void main() {
  ...
  setUpAll(() {
    ...
    registerFallbackValue(MovementRequest.walk(entityId: 0, direction: Vector2.zero(), speed: 0));
  });
  ...
  setUp(() {
    ...
    // No need to setup mocks for overridden properties
  ...
  group('PHY-3.2.3 Player State Management Tests', () {
    group('Retry Mechanism', () {
      test('should retry movement with fallback speed on failure', () async {
        ...
        when(() => mockMovementCoordinator.handleMovementInput(
          any(), any(), any(),
        ),).thenAnswer((_) async => MovementResponse.failed(
          request: MovementRequest.walk(entityId: 123, direction: Vector2(-1, 0), speed: 100),
          reason: 'blocked',
      ...
      test('should use emergency fallback after retry failures', () async {
        ...
        when(() => mockPhysicsCoordinator.requestMovement(
          any(), any(), any(),
        ),).thenAnswer((_) async => MovementResponse.success(
          request: MovementRequest.walk(entityId: 123, direction: Vector2.zero(), speed: 30),
        ...
        verify(() => mockPhysicsCoordinator.requestMovement(
          any(), any(), any(),
        ),).called(1);
      });
    });
    ...
    group('Respawn State', () {
      test('should create proper RespawnState on respawn', () async {
        ...
        when(() => mockPhysicsCoordinator.getPosition(any()))
            .thenAnswer((_) async => Vector2(100, 1500)); // Below fall threshold
        when(() => mockPhysicsCoordinator.isGrounded(any()))
        ...
        verify(() => mockPhysicsCoordinator.clearAccumulatedForces(any())).called(greaterThanOrEqualTo(1));
      });
    ...
    group('Movement Request Integration', () {
      test('should create PlayerMovementRequest with proper metadata', () async {
        ...
        when(() => mockMovementCoordinator.handleMovementInput(
          ...
          capturedResponse = MovementResponse.success(
            request: MovementRequest.walk(entityId: 123, direction: direction, speed: speed),
      ...
      test('should track action sequences for combo detection', () async {
        ...
        when(() => mockMovementCoordinator.handleJumpInput(
          any(), any(), variableHeight: any(named: 'variableHeight'),
        ),).thenAnswer((_) async => MovementResponse.success(
